...
async function handleChatToSpeech(ws, userId) {
  console.log('Setting up Chat-to-Speech mode for user:', userId);
...
- Say numbers and dates clearly (e.g., "March 15th, 2024" not "2024-03-15")
- Spell out abbreviations when needed for clarity
- Use verbal separators like "Next," or "Also," for lists
- Keep responses reasonably concise for comfortable listening
      ...
      if (message.type === 'send_message') {
        ...
        try {
          ...
          if (response.toolCalls) {
            ...
            finalResponseText = response.finalResponse;
          } else {
          ...
          // Generate speech with caring, supportive tone
          const audioBuffer = await openaiService.generateSpeech(finalResponseText, 'alloy', 'DEFAULT');
          ...
          console.error('Chat-to-Speech error:', error);
          ws.send(JSON.stringify({
            type: 'error',
...
module.exports = { handleChatToSpeech };