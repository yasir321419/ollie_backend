...
async function handleSpeechToChat(ws, userId) {
  console.log('Setting up Speech-to-Chat mode for user:', userId);
...
- Parse natural speech patterns (e.g., "tomorrow at 3" = schedule for 3 PM tomorrow)
- Be forgiving of speech-to-text errors
- Provide confirmations for actions taken (e.g., "Got it! I've created your task to...")
- If multiple interpretations are possible, choose the most likely one and confirm
...
Powered by GPT-4.1-mini-2025-04-14 for fast, intelligent responses.`
  ...
  const messageHandler = async (data) => {
    try {
      ...
      ws.send(JSON.stringify({
        type: 'error',
        message: 'Error processing your speech'
      }));
    }
  };
...
module.exports = { handleSpeechToChat };